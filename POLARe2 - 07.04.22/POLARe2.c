/* POLARe2.c:
 ...
 Screening function (HF): Cox and Bonham (1967) (Z<=54), Salvat et al. (1987) (Z>54)
 Exchange potential energy (HF) (only for electron projectiles): Furness and McCarty (1973)
 Polarization potential energy: Buckingham
 Correlation potential energy: Perdew and Zunger (1981) (electrons), Jain (1990) (positrons)
...
char in1[20],in2[20],in3[20],in4[20],in5[20],in6[20],in7[20],in8[20],in9[20];
double A1,A2,A3,A4,A5,A6,A7,A8,A9,A10;
double alp1,alp2,alp3,alp4,alp5,alp6,alp7,alp8,alp9,alp10;
double Z,T,rho,Z0,Z1,Z2,Z3,rd,rmax,zz;
double alphad;
double x,j,jj,n,nn,E,Er,W,Z0,Z1,Z2,Z3,p0,p1,p2,p3,K,KF,KL,pl,rmax,r[1000000],V[1000000],T1[1000000],T2[1000000],th[1000000],sd[1000000];
double c,td1,td2;
...
double s;
{
    ...
    double b2,d2;
    double rs3,rs,rsp,ptcoe,ptcop,ptpol;
...
// Correlation potential energy for electrons in eV (Perdew and Zunger, 1981)
    
    rs3=3./(4.*pi*rho);
    rs=pow(rs3,.333333333)/azero;
    
    if (rs<1.)
    {
        ptcoe=-(ebye/azero)*(0.0311*log(rs)-0.0584+0.00133*rs*log(rs)-0.0084*rs); // Perdew and Zunger
    }
    else
    {
        ptcoe=-(ebye/azero)*0.1423*(1.+(7./6.)*1.0529*sqrt(rs)+(4./3.)*0.3334*rs)/((1.+1.0529*sqrt(rs)+0.3334*rs)*(1.+1.0529*sqrt(rs)+0.3334*rs)); // Perdew and Zunger
    }
    
// Correlation potential energy for positrons in eV (Jain, 1990)
    
    if (rs<0.302)
    {
        ptcop=(ebye/azero)*(0.5835+0.9*sqrt(rs)+(0.00255*log(rs)-0.0575)*log(rs)); // Jain
    }
    else
    {
        if (rs<0.56)
        {
            ptcop=(ebye/azero)*(0.461525-0.04549/(rs*rs)); // Jain
        }
        else
        {
            if (rs<8)
            {
                rsp=rs+2.5;
                ptcop=(ebye/azero)*((-4.3637/(rsp*rsp*rsp))+((-6.5755+0.4776*rs)/(rsp*rsp))+(1.43275/rsp)-0.3149); // Jain
            }
            else
            {
                ptcop=(ebye/azero)*((-15375.8679*pow(rs,-6.))+(44.5047*pow(rs,-3))-0.262); // Jain
            }
        }
    }
...
// Buckingham polarization potential energy in eV (Salvat, 2003)
    
    b2=(E-50.)/16.;
    if (b2<1.) b2=1.;
    d2=sqrt(0.5*b2*alphad*azero*(pow(Z,-.33333333)));
    ptb=-0.5*alphad*ebye/((s*s*hbardbymc*hbardbymc+d2)*(s*s*hbardbymc*hbardbymc+d2));

// Polarization potential energy in eV
...
//Potential energy in eV
    ...
    {
        return (-pt+ptexch+ptpol)/mcbyc;
    }
    else //positron beam
    {
        return (pt+ptpol)/mcbyc;
    }
}
...
//Input: Polarizability in A^3 (Schwerdtfeger, 2014)
    
    zz=0;
    cid3=fopen("INPUT/polarizability.txt","r");
    while(zz!=Z)
    {
        fscanf(cid3,"%lg%lg",&zz,&alphad);
    }
    fclose(cid3);
...
//Energy, momentum, rmax
    ...
    rcp=rmax;
    DELTA=1;
    while((DELTA>epsilon)&&(rcp>epsilon))
    {
        vrcp=vi(rcp);
        DELTA=fabs(ptb-ptco);
        rcp=rcp-epsilon;
    }
    printf("\nrmax (A) = %.2f",rmax*hbardbymc);
    printf("\nrcp (A) = %.2f\n",rcp*hbardbymc);
...
//Calculation of the radial atomic density
    ...
    while (rd<rmax)
    {
        ...
        //fprintf(codAD,"%lg %lg \n",rd*hbardbymc/azero,azero*ad(rd)); // radius in azero; density in azero^-1
        rd=rd+h/2.;
    }
    
    fprintf(codST,"\n%s = %lg\n%s = %lg\n%s = %ld\n%s = %lg\n\n",in1,Z,in2,E,in3,electron,in7,azimuth*180./pi);
    fprintf(codST,"%s = %lg\n%s = %lg\n%s = %lg\n\n",in4,Px,in5,Py,in6,Pz);
    fprintf(codST,"alphaD (Aˆ3) = %lg\n\n",alphad);
    fprintf(codST,"P = %lg\n\n",P);
    fprintf(codST,"Pn = %lg \n\n",Pn);
...
//Gnuplot figures
    ...
    fprintf(gnuplot, "set xlabel '{/Symbol q} (deg)' \n");
    fprintf(gnuplot, "set ylabel 'd{/Symbol s}/d{/Symbol W} (10^{-16}cm^2/sr)' \n");
    ...
    fprintf(gnuplotS, "set termoption enhanced\n");
    fprintf(gnuplotS, "set xlabel '{/Symbol q} (deg)' \n");
    ...
    fprintf(gnuplotT, "set termoption enhanced\n");
    fprintf(gnuplotT, "set xlabel '{/Symbol q} (deg)' \n");
    ...
    fprintf(gnuplotU, "set termoption enhanced\n");
    fprintf(gnuplotU, "set xlabel '{/Symbol q} (deg)' \n");
    ...
    fprintf(gnuplotDp, "set termoption enhanced\n");
    fprintf(gnuplotDp, "set xlabel '{/Symbol q} (deg)' \n");
    fprintf(gnuplotDp, "set ylabel 'd{/Symbol s}/d{/Symbol W} (10^{-16}cm^2/sr)' \n");
    ...
    fprintf(gnuplotCOMP, "set termoption enhanced\n");
    fprintf(gnuplotCOMP, "set xlabel '{/Symbol q} (deg)' \n");
    fprintf(gnuplotCOMP, "set ylabel 'd{/Symbol s}/d{/Symbol W} (10^{-16}cm^2/sr)' \n");
    ...
    fprintf(gnuplotP, "set termoption enhanced\n");
    fprintf(gnuplotP, "set xlabel '{/Symbol q} (deg)' \n");
